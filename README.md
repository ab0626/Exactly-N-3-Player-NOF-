...
## Pedagogical proxies (“application layer” knobs)

These features exist to make **proof-level themes** discussable in code, without claiming the repo re-implements the paper’s inner iterations (Bohr-set maintenance, relative sifting lemmas, etc.).

- **Asymmetric diagonal tolerance (`--asymmetric`, `--xy-tol`, `--diag-tol`)**: relaxes Charlie’s certificate consistency checks vs Alice/Bob. This is a **storytelling proxy** for proof sketches where different sides of a product structure satisfy different “spread/softness” requirements (see the paper’s discussion of asymmetric notions and delicate “great pair” setups in the proof overview).
- **Sifting noise (`--sifting-noise`, `--bohr-dim`)**: randomly perturbs the two coordinates each player sees. `--bohr-dim` is a **tunable noise amplifier dial** for sweeps/heatmaps (not a literal Bohr rank implementation in this repo).
- **Theorem-shaped reference curve (`--theorem-c`, `--theorem-exp`)**: adds a visualization baseline of the form \(\exp(-c(\log N)^{1/E})\), defaulting to \(E=600\) to match **Theorem 1.1’s displayed exponent family**. The constant `c` is **not tuned** to match the paper’s unspecified \(c\).
- **Heatmap sweep (`--sweep-kind heatmap`)**: produces a **toy reliability surface** over thinning vs the noise dial, outputting `outputs/heatmap_sweep.csv` and `docs/figures/heatmap_reliability.svg`. This is **not** Theorem 7.9’s corner probability output; it is a **phenomenological** diagnostic for the simulator.
...
Optional reliability heatmap (phenomenological):

![Reliability heatmap (toy protocol)](docs/figures/heatmap_reliability.svg)
...
Regenerate the heatmap artifact:

```bash
python exactly_n_protocol.py --mode sweep --sweep-kind heatmap --heatmap-rounds 400 --seed 0 --output-dir outputs --figures-dir docs/figures
```
...
- `--sweep-kind`:
  - `grid` (default): full multi-parameter sweep + line-plot SVGs.
  - `heatmap`: writes `outputs/heatmap_sweep.csv` + `docs/figures/heatmap_reliability.svg`.
- `--theorem-c`, `--theorem-exp`: parameters for the **Theorem 1.1-shaped** reference curve \(\exp(-c(\log N)^{1/E})\) (visualization only).
- `--asymmetric`, `--xy-tol`, `--diag-tol`: asymmetric certificate tolerance proxy (mostly affects nondeterministic certificate checks).
- `--sifting-noise`, `--bohr-dim`: local-view noise + noise amplifier dial.
- `--heatmap-domain`, `--heatmap-base`, `--heatmap-modulus`, `--heatmap-rounds`, `--heatmap-thinnings`, `--heatmap-dims`: heatmap grid controls.
...
- heatmap artifacts (only for `--sweep-kind heatmap`):
  - `outputs/heatmap_sweep.csv`
  - `docs/figures/heatmap_reliability.svg`
...
- `outputs/heatmap_sweep.csv`:
  - rows from `--sweep-kind heatmap` (thinning vs noise dial).
...
- committed SVG figures (recommended for GitHub):
  ...
  - `docs/figures/heatmap_reliability.svg`: toy reliability heatmap (if generated).
- optional PNG plots:
...
- **Reference/theory columns**:
  ...
  - `ref_theorem11_density_shape`: **Theorem 1.1-shaped** density decay proxy \(\exp(-c(\log N)^{1/600})\) (constants not tuned).
  - `legacy_new_density_curve_exp_logN_0p2`: older heuristic curve kept for backwards compatibility in CSVs.
